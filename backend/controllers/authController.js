...
exports.forgotPassword = catchAsyncError(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      email: user.email,
      subject: "ShopIt password recover",
      message,
    });
    res.status(200).json({
      success: true,
...
exports.resetPassword = catchAsyncerrors(async (req, res, next) => {
  ...
  //finding the user with the same passwordtoken
...
exports.updatePassword = catchAsyncError(async (req, res, next) => {
  ...
  //password coming from body through postman
  user.password = req.body.password;
  await user.save();
  sendToken(user, 200, res);
});
...
//get user details ie specific user by id => /api/v1/admin/user/:id
exports.getUserDetails = catchAsyncError(async (req, res, next) => {
  const user = await User.findById(req.params.id);

  if (!user) {
    return next(
      new ErrorHandler(`User does not found with this ${req.params.id}`)
    );
  }
  res.status(200).json({
    sucess: true,
...
exports.deleteUser = catchAsyncError(async (req, res, next) => {
  ...
  // remove avatar fron cloudinary : TODO
  await user.remove();
  res.status(200).json({
    sucess: true,
  });
